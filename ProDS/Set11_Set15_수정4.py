...
# 가장 많은 제품이 팔린 날짜 -> 날짜 추출
pos.columns
# ['Member_number', 'Date', 'itemDescription']
top_date=pos['Date'].value_counts().idxmax()


# 추출된 날짜의 기준으로 필터링을 한 후 가장 많이 팔린 제품의 판매 개수

q1=pos[pos.Date==top_date]

q1['itemDescription'].value_counts().max()
...
q2.Date.dtype
pd.to_datetime(q2.Date).dtype

q2['month']=pd.to_datetime(q2.Date).dt.month
q2['day']=pd.to_datetime(q2.Date).dt.day_name(locale='ko_kr')


# 금/토 이냐 아니냐 새로운 변수 생성

q2['week']=0
q2.loc[q2.day.isin(['금요일', '토요일']), 'week']=1
q2['week'].value_counts()

# pos와 list1 데이터셋 결합을 통해 주류 유무 정보 추출
q2.columns
# ['Member_number', 'Date', 'itemDescription', 'month', 'day', 'week']
list1.columns
# ['prod_id', 'prod_nm', 'alcohol', 'frozen']
q2_merge=pd.merge(q2, list1, left_on='itemDescription',
         right_on='prod_nm',
         how='left')
...
# 요일의 일별 주류제품 구매 제품 수 평균이 서로 다른지 비교하기 위해 
# 독립 2표본 t검정

q2_tab=pd.pivot_table(q2_merge2,
                      index='Date',
                      columns='week',
                      values='alcohol',
                      aggfunc='sum')
...
top10_data=pos['itemDescription'].value_counts()

top10_list=top10_data.nlargest(10).index

q3=pos[pos.itemDescription.isin(top10_list)].reset_index(drop=True)

q3['day']=pd.to_datetime(q3.Date).dt.day_name(locale='ko_kr')


# 분산분석용 데이터셋 만들기
from scipy.stats import f_oneway   # 분산분석용 함수
from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm
...
anova=ols('itemDescription~C(day)', q3_tab).fit()
anova.summary()

aov_out=anova_lm(anova)
aov_out['PR(>F)'][0]

# 정답 : 0.5181278037418005 -> 0.52

#                     df        sum_sq    mean_sq        F    PR(>F)
# 그룹간 C(day)      6.0    219.527473  36.587912  0.86853  0.518128
# 그룹내 Residual  357.0  15039.076923  42.126266      NaN       NaN

# 결론: 귀무가설 기각할 수 없음, 집단들끼리 평균은 차이가 없다

# 사후 검정 : 집단 간의 차이가 난다는 의사결정을 했을 경우(귀무가설 기각된 경우)
# 실제 어떤 집단이 평균이 같은지 아닌지 검정 
from statsmodels.stats.multicomp import pairwise_tukeyhsd 

print(pairwise_tukeyhsd(q3_tab.itemDescription, q3_tab.day))
