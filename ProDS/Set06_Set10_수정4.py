...
g1=data6[data6.waterfront==1]['price'].mean()
g0=data6[data6.waterfront==0]['price'].mean()
...
x_var=data6.columns.drop(['id','date','zipcode','price'])

from statsmodels.formula.api import ols  # 상수항 포함

# from statsmodels.api import OLS, add_constant # OLS 상수항 미포함
# add_constant을 통해 상수항 계산할 수 있는 입력 변수 추가 후 OLS 함수 사용

# ols(식, 데이터셋).fit()
# 식: 'y~x1+x2+C(x3)'  => 범주형 변수는 자동적으로 더미로 변경됨

form = 'price~'+'+'.join(x_var)

ols1=ols(form, data=data6).fit()
...
# Time:                        14:40:05   Log-Likelihood:                -58960.
...
# Kurtosis:                      17.051   Cond. No.                     3.03e+17
# ==============================================================================


q3_out=ols1.pvalues.drop('Intercept')[ols1.pvalues.drop('Intercept') < 0.05]
len(q3_out)

(ols1.params[q3_out.index] < 0).sum()
...
q1_tab=pd.pivot_table(q1, index='model',
                      ...
                      # aggfunc={'age_in_days':np.sum,
                      #          'km':np.mean})
# 일평균
q1_tab.columns
q1_tab['mean']=q1_tab['km']/q1_tab['age_in_days']

# 최대-최소
q1_ans=q1_tab['mean'].max() - q1_tab['mean'].min()

round(q1_ans, 2)
# 정답: 1.022810996086104 -> 1.02
...
min_g= q2[q2.model==q1_tab['mean'].idxmin()]['km_per_day']
max_g= q2[q2.model==q1_tab['mean'].idxmax()]['km_per_day']

from scipy.stats import ttest_ind, bartlett # 등분산 검정

bartlett(max_g, min_g)
# BartlettResult(statistic=2.8320958263094305, pvalue=0.09239770665253491)
# H0: 등분산
# H1: 이분산
# => 결론: 등분산

q2_out=ttest_ind(max_g, min_g, equal_var=True)
q2_out.pvalue
...
new_data=pd.DataFrame({'engine_power': [51], 
                       'age_in_days': [400], 
                       'km':[9500] })
