...
### 함수 인수를 변수로 사용하면 값이 전달되는게 아니고 참조 주소가 전달되면서 같은 메모리를 참조 하게 된다.
## 일반 변수는 전달된 매개변수에 값을 변경하면 메모리주소가 달라지면서 다른 변수로 생성
## 리스트 변수는 전달된 매개변수의 일부값을 변경하면 원본도 변경시키고, 전체를 변경하면  메모리 주소가 달라진다.
